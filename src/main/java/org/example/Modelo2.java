...
// 1. Crear el FileSplit
        ...
        MultiLayerConfiguration config = getConfigForSecondModel(channels, outputNum, height, width);
        ...
        File evalFile = new File("src/main/resources/saved_models/model2/evaluation.json");
...
// Guardar el modelo entrenado
        File modeloGuardado = new File("src/main/resources/saved_models/model2/model.zip");
        modeloGuardado.getParentFile().mkdirs();
        ModelSerializer.writeModel(model, modeloGuardado, true);
    }

    private static MultiLayerConfiguration getConfigForSecondModel(int channels, int outputNum, int height, int width) {
